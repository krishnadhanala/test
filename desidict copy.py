...
client = pymongo.MongoClient('mongodb://dd:Testvpc1@desi.cuycutte3zdq.us-east-1.docdb.amazonaws.com:27017/?ssl=true&ssl_ca_certs=rds-combined-ca-bundle.pem&retryWrites=false',tlsAllowInvalidCertificates=True)
...
flow = Flow.from_client_secrets_file(
    ...
    redirect_uri="http://desiart.com/user/callback"
)
...
def userlogin():
    ...
    session["state"] = state
    # print(session.keys(),'sessionkeys', flush = True)
    return redirect(authorization_url)

@app.route('/user/callback', methods=['GET'])
def callback():
    flow.fetch_token(authorization_response=request.url)
    print('hererere',flush = True)
    print(session["state"], request.args["state"] ,'state', flush = True)
...
@app.route("/adminDashboard/"+"admin", methods=['GET'])
def dash():
